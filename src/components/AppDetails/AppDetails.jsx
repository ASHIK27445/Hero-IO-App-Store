...
const AppDetails = () => {
  ...
  useEffect(()=>{
    ...
    const exist = data?.find(item => item.id == appID)
    if(exist){
          setApp(exist)
          const storingApp = getStoredApp()
          const alreadyInstalled = storingApp.some(item => item.id == exist.id)
  ...
  return (
    <div className="p-4 font-inter lg:max-w-[1140px] mx-auto ">
        <div className="flex gap-15 mt-20 mb-10">
            <div className=" hidden md:block w-[180px] md:w-[220px] lg:w-[250px] h-[180px] md:h-[220px] lg:h-[250px] border-1 border-red-700 rounded-lg p-4">
                <img className="w-full h-full object-contain" src={app?.image}/>
            </div>
            <div>
                <div className="mb-10 flex items-center md:flex-none">
                    <div className=" visible md:hidden w-[80px] h-[80px] shadow-sm shadow-gray-400 rounded-lg p-4 mr-5">
                        <img className="w-full h-full object-contain" src={app?.image}/>
                    </div>
                    <div>
