...
const Apps = () => {
    ...
    const [filterSpin, setFilterSpin] = useState(data)
    ...
    return(
        <div>
            ...
            <div className="flex justify-between items-center my-4 max-w-[1064px] mx-auto gap-5">
                <h3 className="text-[#001931] font-semibold text-lg px-4 md:px-0">({filterSpin.length}) Apps Found</h3>
            ...
            <div className="grid max-w-[400px] grid-cols-2 sm md:grid-cols-3 md:max-w-[800px] lg:grid-cols-4 lg:max-w-[1064px] gap-4 mx-auto mb-10">
                { loading ? 
                    <div className="col-span-full flex justify-center mt-[75px] mb-[50px]"> <HashLoader color="red"></HashLoader></div> : 
                    filterSearch.length > 0 ?
                    filterSearch.map(appData => <AppCard key={appData.id} appData={appData}></AppCard>) :
                    <div className=" mt-15 mb-10 font-bold text-lg p-2 rounded-lg flex justify-center col-span-full mx-auto border-2 border-black text-[#DC143C]"><p>No App Found</p></div>
                }
            </div>
        </div>
    )
